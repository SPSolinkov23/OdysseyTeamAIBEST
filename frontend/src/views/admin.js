...
function row(u, i) {
    ...
    return (
        ...
        "<div><div class=\"flex flex-wrap items-center gap-2\"><h3 class=\"font-display text-base font-semibold text-slate-800 dark:text-slate-100\">" + UI.escape(u.name) + "</h3>" +
        ...
        "<p class=\"mt-1 text-sm text-slate-500 dark:text-slate-400\"><i class=\"fa-regular fa-envelope mr-1.5\"></i>" + UI.escape(u.email) + "<span class=\"mx-2 text-slate-300 dark:text-slate-600\">•</span><i class=\"fa-regular fa-clock mr-1.5\"></i>" + UI.escape(I18n.t("admin.applied", { time: UI.fmtRelative(u.createdAt) })) + "</p></div></div>" +
...
export async function admin() {
    ...
    const html =
        "<section class=\"bg-hero-grid bg-grid-dots border-b border-slate-200/70 bg-white dark:border-slate-700/70 dark:bg-slate-900\"><div class=\"container-app py-10\" data-aos=\"fade-right\">" +
        "<span class=\"badge bg-brand-50 text-brand-700 ring-brand-200 dark:bg-brand-900/30 dark:text-brand-400 dark:ring-brand-800\"><i class=\"fa-solid fa-shield-halved\"></i>" + I18n.t("admin.badge") + "</span>" +
        "<h1 class=\"mt-3 font-display text-3xl font-bold text-slate-800 dark:text-slate-100\">" + I18n.t("admin.title") + "</h1>" +
        "<p class=\"mt-1 text-slate-500 dark:text-slate-400\">" + I18n.t("admin.subtitle") + "</p></div></section>" +
