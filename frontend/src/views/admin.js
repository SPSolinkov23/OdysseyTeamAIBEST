...
function emptyState() {
    return UI.empty({
        ...
        text: "When someone applies to become an organizer, they'll show up here.",
    });
}
...
function row(u, i) {
    ...
    return (
        ...
        '<span class="badge bg-amber-100 text-amber-700 ring-amber-200"><i class="fa-solid fa-user-clock"></i>Pending</span></div>' +
        '<p class="mt-1 text-sm text-slate-500"><i class="fa-regular fa-envelope mr-1.5"></i>' + UI.escape(u.email) + '<span class="mx-2 text-slate-300">•</span><i class="fa-regular fa-clock mr-1.5"></i>Applied ' + UI.escape(UI.fmtRelative(u.createdAt)) + "</p></div></div>" +
        '<div class="flex flex-wrap items-center gap-2">' +
        '<button data-approve="' + u.id + '" class="btn-primary btn-sm"><i class="fa-solid fa-check"></i> Approve</button>' +
        '<button data-reject="' + u.id + '" class="btn-ghost btn-sm text-rose-600 hover:bg-rose-50"><i class="fa-solid fa-xmark"></i> Reject</button>' +
        "</div></div></div>"
    );
}
...
export async function admin() {
    ...
    const html =
        ...
        '<span class="badge bg-brand-50 text-brand-700 ring-brand-200"><i class="fa-solid fa-shield-halved"></i>Admin</span>' +
        '<h1 class="mt-3 font-display text-3xl font-bold text-slate-800">Organizer approvals</h1>' +
        '<p class="mt-1 text-slate-500">Review and decide on pending organizer requests.</p></div></section>' +
...
async function decide(root, id, action) {
    ...
    const ok = await UI.confirm({
        title: isApprove ? "Approve organizer" : "Reject organizer",
        text: isApprove ? "This user will gain organizer access and be notified." : "This user will stay a student and be notified.",
    ...
    try {
        ...
        UI.toast(isApprove ? "Organizer approved. 🎉" : "Request rejected.", isApprove ? "success" : "info");
