(function () {
    ...
    Views.organizer = async function () {
        const events = await window.API.listOrganizerEvents();
        const published = events.filter((e) => e.status === "PUBLISHED").length;
        const drafts = events.filter((e) => e.status === "DRAFT").length;
        const totalRegs = events.reduce((a, e) => a + e.confirmedCount + e.waitlistCount, 0);

        const stats = [
            { icon: "fa-layer-group", label: "Total events", value: events.length, color: "brand" },
        ...
        const rows = events.length
            ? events.map((e, idx) => eventRow(e, idx)).join("")
            : window.UI.empty({ icon: "fa-calendar-plus", title: "No events yet", text: "Create your first event and publish it for students.", actionHtml: '<a href="#/organizer/new" class="btn-primary"><i class="fa-solid fa-plus"></i> New event</a>' });

        const html =
            '<section class="hero-grid border-b border-slate-200/70 bg-white"><div class="container-app py-10"><div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between" data-aos="fade-right">' +
    ...
    function eventRow(e, idx) {
        const cat = window.UI.categoryMeta(e.category);
        ...
        return (
            '<div class="card p-5" data-aos="' + window.UI.aos(idx) + '" data-aos-delay="' + idx * 50 + '"><div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">' +
            ...
            '<div><div class="flex flex-wrap items-center gap-2"><h3 class="font-display text-base font-semibold text-slate-800">' + window.UI.escape(e.title) + "</h3>" + window.UI.eventBadge(e.status) + "</div>" +
            '<p class="mt-1 text-sm text-slate-500"><i class="fa-regular fa-calendar mr-1.5"></i>' + window.UI.escape(window.UI.fmtRange(e.startsAt, e.endsAt)) + '<span class="mx-2 text-slate-300">\u2022</span><i class="fa-solid fa-users mr-1.5"></i>' + e.confirmedCount + "/" + e.capacity + " registered" + (e.waitlistCount ? '<span class="mx-2 text-slate-300">\u2022</span><i class="fa-solid fa-hourglass-half mr-1.5 text-amber-500"></i>' + e.waitlistCount + " waiting" : "") + "</p></div></div>" +
    ...
    async function publish(id) {
        const ok = await window.UI.confirm({ title: "Publish event", text: "Students will be able to see it and register.", confirmText: "Publish", icon: "question" });
        if (!ok) return;
        try {
            await window.API.publishEvent(window.Auth.current().id, id);
            window.UI.toast("Event published! \ud83d\ude80", "success");
            window.App.rerender();
        } catch (e) {
    ...
    async function cancelEvent(id) {
        const ok = await window.UI.confirm({ title: "Cancel event", text: "All registrants will be notified and their registrations cancelled.", confirmText: "Cancel event", danger: true, icon: "warning" });
        if (!ok) return;
        try {
            await window.API.cancelEvent(window.Auth.current().id, id);
            window.UI.toast("Event cancelled. Notifications sent.", "info");
            window.App.rerender();
        } catch (e) {
            window.UI.toast(e.message, "error");
        }
    ...
    Views.organizerForm = async function (id) {
        const user = window.Auth.current();
        let ev = null;
        if (id) {
            ev = await window.API.getEvent(id);
            if (!ev) return { html: window.UI.guard("Event not found", "It may have been removed."), onMount: null };
            if (ev.organizerId !== user.id) return { html: window.UI.guard("No access", "You can only edit your own events."), onMount: null };
            if (ev.status === "CANCELLED") return { html: window.UI.guard("Cancelled event", "Cancelled events cannot be edited."), onMount: null };
        }
        ...
        const html =
            ...
            field("Title", '<input id="p-title" name="title" class="input" value="' + window.UI.escape(d.title) + '" placeholder="e.g. Intro to Web Programming">') +
            ...
            "</div>" +
            field("Description", '<textarea id="p-description" name="description" rows="4" class="input" placeholder="What will happen at the event?">' + window.UI.escape(d.description) + "</textarea>") +
            '<div class="grid gap-5 sm:grid-cols-2">' +
            ...
            "</div>" +
            '<div class="grid gap-5 sm:grid-cols-2">' +
            field("Location", '<input id="p-location" name="location" class="input" value="' + window.UI.escape(d.location) + '" placeholder="Room 204">') +
            field("Link (optional)", '<input id="p-url" name="url" class="input" value="' + window.UI.escape(d.url) + '" placeholder="https://...">') +
            "</div>" +
    ...
    function previewCard(data) {
        const cat = window.UI.categoryMeta(data.category);
        const cap = parseInt(data.capacity, 10) || 0;
        const range = data.startsAt ? window.UI.fmtRange(data.startsAt, data.endsAt) : "Date and time";
        return (
            ...
            '<span class="badge bg-white/20 text-white ring-white/30">' + window.UI.escape(data.category || "Event") + "</span></div>" +
            '<div class="p-5"><h3 class="font-display text-lg font-semibold text-slate-800">' + (window.UI.escape(data.title) || '<span class="text-slate-400">Event title</span>') + "</h3>" +
            '<div class="mt-2 space-y-1.5 text-sm text-slate-500"><p><i class="fa-regular fa-calendar mr-2 text-brand-500"></i>' + window.UI.escape(range) + "</p>" +
            '<p><i class="fa-solid fa-location-dot mr-2 text-brand-500"></i>' + (window.UI.escape(data.location) || "Location") + "</p>" +
            '<p><i class="fa-solid fa-users mr-2 text-brand-500"></i>0/' + cap + " registered</p></div>" +
            '<div class="mt-4">' + window.UI.progressBar(0, cap, "brand") + "</div></div></article>"
        );
    }
    ...
    function bindForm(root, user, ev) {
        ...
        render();
        if (window.Choices) {
            new window.Choices(root.querySelector("#p-category"), { searchEnabled: false, shouldSort: false, itemSelectText: "", allowHTML: false });
        }
        ...
        form.addEventListener("submit", async (e) => {
            ...
            try {
                if (ev) {
                    await window.API.updateEvent(user.id, ev.id, data);
                    window.UI.toast("Changes saved.", "success");
                } else {
                    await window.API.createEvent(user.id, data);
                    window.UI.toast("Draft created.", "success");
                }
    ...
    Views.eventRegistrations = async function (id) {
        const user = window.Auth.current();
        const ev = await window.API.getEvent(id);
        if (!ev) return { html: window.UI.guard("Event not found", "It may have been removed."), onMount: null };
        if (ev.organizerId !== user.id) return { html: window.UI.guard("No access", "You can only view registrations for your own events."), onMount: null };

        const attendees = await window.API.getEventAttendees(id);
        ...
        const html =
            ...
            '<div><div class="flex items-center gap-2">' + window.UI.eventBadge(ev.status) + '<span class="text-sm text-slate-500">' + window.UI.escape(ev.category) + "</span></div>" +
            '<h1 class="mt-2 font-display text-2xl font-bold text-slate-800">' + window.UI.escape(ev.title) + "</h1>" +
            '<p class="mt-1 text-sm text-slate-500"><i class="fa-regular fa-calendar mr-1.5"></i>' + window.UI.escape(window.UI.fmtRange(ev.startsAt, ev.endsAt)) + "</p></div>" +
    ...
    function peopleList(items, isWaitlist) {
        if (!items.length) {
            return window.UI.empty({ icon: isWaitlist ? "fa-hourglass-end" : "fa-user-slash", title: isWaitlist ? "Empty waitlist" : "No registrations yet", text: isWaitlist ? "When capacity fills up, new registrations will appear here." : "Share the event to start collecting registrations." });
        }
        return (
            ...
            items
                .map((it, idx) => {
                    ...
                    return (
                        '<div class="flex items-center gap-4 p-4" data-aos="' + window.UI.aos(idx) + '" data-aos-delay="' + idx * 40 + '">' +
                        ...
                        '<span class="flex h-10 w-10 flex-none items-center justify-center rounded-full bg-brand-100 text-sm font-bold text-brand-700">' + window.UI.escape(initials) + "</span>" +
                        '<div class="min-w-0 flex-1"><div class="truncate font-medium text-slate-800">' + window.UI.escape(u.name) + '</div><div class="truncate text-sm text-slate-500">' + window.UI.escape(u.email) + "</div></div>" +
                        '<div class="text-right text-xs text-slate-400"><i class="fa-regular fa-clock mr-1"></i>' + window.UI.fmtRelative(it.registration.registeredAt) + "</div></div>"
                    );
                })
                .join("") +
            "</div>"
    ...
    Views.preview = async function (id) {
        const user = window.Auth.current();
        const ev = await window.API.getEvent(id);
        if (!ev) return { html: window.UI.guard("Event not found", "It may have been removed."), onMount: null };
        if (ev.organizerId !== user.id) return { html: window.UI.guard("No access", "You can only preview your own events."), onMount: null };
        ...
        const html =
            ...
            '<div class="overflow-hidden rounded-3xl"><div class="relative flex h-44 items-end bg-gradient-to-br from-' + cat.color + "-500 to-" + cat.color + '-700 p-6 text-white sm:h-56"><div class="pointer-events-none absolute inset-0 mesh opacity-50"></div><span class="pointer-events-none absolute right-6 top-6 text-6xl text-white/20"><i class="fa-solid ' + cat.icon + '"></i></span>' +
            '<div class="relative"><span class="badge bg-white/20 text-white ring-white/30">' + window.UI.escape(ev.category) + '</span><h1 class="mt-3 font-display text-2xl font-bold sm:text-3xl">' + window.UI.escape(ev.title) + "</h1></div></div></div>" +
            '<div class="mt-6 card p-6"><h2 class="font-display text-lg font-semibold text-slate-800">Description</h2><p class="mt-2 whitespace-pre-line leading-relaxed text-slate-600">' + (window.UI.escape(ev.description) || "\u2014") + "</p></div></div>" +
            ...
            window.UI.progressBar(ev.confirmedCount, ev.capacity, "brand") +
