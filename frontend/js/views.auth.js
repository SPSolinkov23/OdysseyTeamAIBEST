(function () {
    ...
    Views.auth = function (mode) {
        ...
        const html =
            ...
            (isLogin ? "" : '<p class="mt-1 text-xs text-slate-400">At least 8 characters.</p>') +
            "</div>" +
    ...
    function bind(root, isLogin) {
        ...
        form.addEventListener("submit", async (e) => {
            ...
            try {
                const user = isLogin ? await window.Auth.login(data.email, data.password) : await window.Auth.register(data);
                afterAuth(user);
            } catch (err) {
    ...
    function afterAuth(user) {
        window.UI.toast("Hi, " + user.name.split(" ")[0] + "!", "success");
